#Script to compare current IP with old IP and sends webhook notification if different (and do nothing if there was no change).
...
$WebhookUrl = "XXXXXXXXXXXXXXXX" #put webhook here
...
$IP_old = Get-Content "$IPAddFile"
    ...
    if(-not($IP_new -eq $IP_old))
    {
        ...
        $body = $("{""text"":""$msg""}")
        Write-Host "$body"
        Invoke-RestMethod -Uri $WebhookUrl -Method Post -ContentType 'application/json' -Body $body -OutFile $slackOutputFile
